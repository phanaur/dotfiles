...
read -rp "¿Instalar drivers Nvidia propietarios? [s/N]: " resp_nvidia
INSTALL_NVIDIA=false
[[ "${resp_nvidia,,}" == "s" ]] && INSTALL_NVIDIA=true
...
read -rp "¿Instalar Google Chrome? [s/N]: " resp_chrome
INSTALL_CHROME=false
[[ "${resp_chrome,,}" == "s" ]] && INSTALL_CHROME=true

read -rp "¿Instalar Steam? [s/N]: " resp_steam
INSTALL_STEAM=false
[[ "${resp_steam,,}" == "s" ]] && INSTALL_STEAM=true
...
if $INSTALL_NVIDIA; then
    ...
    sudo sed -i 's/^GRUB_CMDLINE_LINUX_DEFAULT="/&nvidia-drm.modeset=1 nvidia-drm.fbdev=1 /' /etc/default/grub 2>/dev/null || \
        warn "No se pudo modificar GRUB automáticamente. Añade 'nvidia-drm.modeset=1 nvidia-drm.fbdev=1' manualmente."
    sudo grub-mkconfig -o /boot/grub/grub.cfg 2>/dev/null || warn "grub-mkconfig falló, puede que uses systemd-boot."

    info "Añadiendo módulos Nvidia al initramfs..."
    sudo sed -i 's/^MODULES=(/MODULES=(nvidia nvidia_modeset nvidia_uvm nvidia_drm /' /etc/mkinitcpio.conf
    sudo mkinitcpio -P

    success "Drivers Nvidia instalados. Reinicia antes de continuar."
else
    header "2. Drivers Nvidia"
    info "Omitiendo drivers Nvidia."
fi

# ── 3. Paquetes base Wayland ──────────────────────────────────────────────────
header "3. Paquetes base Wayland"
...
if $INSTALL_NIRI; then
    header "6. Niri"
    $AUR -S --noconfirm --needed niri xwayland-satellite
...
# ── 9. Software opcional ──────────────────────────────────────────────────────
header "9. Software opcional"

if $INSTALL_CHROME; then
    info "Instalando Google Chrome..."
    $AUR -S --noconfirm --needed google-chrome
...
header "10. Ficheros de configuración"
...
bind=NONE,Print,spawn,/home/USERPLACEHOLDER/.config/mango/screenshot.sh
bind=SUPER,Print,spawn,/home/USERPLACEHOLDER/.config/mango/screenshot-area.sh
...
exec-once=/home/USERPLACEHOLDER/.config/mango/autostart.sh
MANGO_EOF

    # Reemplazar USERPLACEHOLDER con el usuario real
    sed -i "s/USERPLACEHOLDER/$(whoami)/g" ~/.config/mango/config.conf
...
MWAYBAR_EOF
    ...
    cp ~/.config/waybar/style.css ~/.config/mango/waybar/style.css
...
RWAYBAR_EOF
    ...
    cp ~/.config/waybar/style.css ~/.config/river/waybar/style.css
